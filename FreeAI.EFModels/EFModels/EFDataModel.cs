...
namespace FreeAI.EFModels.EFModels;
...
public partial class EFDataModel : DbContext
{
    ...
    // => optionsBuilder.UseSqlite("Data Source=C:\\Working\\FreeAI.db");
    // => optionsBuilder.UseMySQL("Server=localhost;Database=FreeAI;User=admin;Password=admin");
    // => optionsBuilder.UseNpgsql("Host=localhost;Database=FreeAI;Username=postgres;Password=admin");
    // => optionsBuilder.UseSqlServer("Data Source=(local);Initial Catalog=FreeAI;Persist Security Info=True;User ID=sa;Password=saPassword;MultipleActiveResultSets=True;TrustServerCertificate=True;");
