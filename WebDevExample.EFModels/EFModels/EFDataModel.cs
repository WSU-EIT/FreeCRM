...
namespace WebDevExample.EFModels.EFModels;
...
public partial class EFDataModel : DbContext
{
    ...
    // => optionsBuilder.UseSqlite("Data Source=C:\\Working\\WebDevExample.db");
    // => optionsBuilder.UseMySQL("Server=localhost;Database=WebDevExample;User=admin;Password=admin");
    // => optionsBuilder.UseNpgsql("Host=localhost;Database=WebDevExample;Username=postgres;Password=admin");
    // => optionsBuilder.UseSqlServer("Data Source=(local);Initial Catalog=WebDevExample;Persist Security Info=True;User ID=sa;Password=saPassword;MultipleActiveResultSets=True;TrustServerCertificate=True;");
